...
def find_best_split(feature_vector, target_vector):
    ...
    * За наличие в функции циклов балл будет снижен. Векторизуйте! :)
    ...
    sorted_indexes = feature_vector.argsort()
    ...
    cumsum_array = np.cumsum(sorted_target)
    
    share_of_true_left = cumsum_array / np.arange(1, target_vector.shape[0])
    
    share_of_true_right = (total_sum - cumsum_array) / np.arange(1, target_vector.shape[0][::-1])
    
    gini_left = 1 - (1 - share_of_true_left) ** 2 - (share_of_true_left) ** 2 
    
    gini_right = 1 - (1 - share_of_true_right) ** 2 - (share_of_true_right) ** 2
    
    total_gini = - np.arange(1, target_vector.shape[0]) / target_vector.shape[0] * gini_left 
    - (target_vector.shape[0] - np.arange(1, target_vector.shape[0])) / target_vector.shape[0] * gini_right
    
    right_index = np.argmax(total_gini)
    
    return thresholds, total_gini, thresholds[right_index], total_gini[right_index]
