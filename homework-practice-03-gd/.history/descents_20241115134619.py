...
class BaseDescent:
    ...
    def calc_loss(self, x: np.ndarray, y: np.ndarray) -> float:
        ...
        if self.loss_function == LossFunction.MSE:
            loss = 1 / (x.shape[0]) * (y - self.predict(x)).T @ (y - self.predict(x))
            return loss
...
class VanillaGradientDescent(BaseDescent):
    ...
    def calc_gradient(self, x: np.ndarray, y: np.ndarray) -> np.ndarray:
        ...
        if self.loss_function == LossFunction.MSE:
            return -2 / x.shape[0] * (y - self.predict(x)).T @ x
...
class StochasticDescent(VanillaGradientDescent):
    ...
    def calc_gradient(self, x: np.ndarray, y: np.ndarray) -> np.ndarray:        
        ...
        indicies = np.random.choice(x.shape[0], size=self.batch_size, replace=False)
        x_batch = x[indicies]
        y_batch = y[indicies]
        ...
        if self.loss_function == LossFunction.MSE:
            ...
            return -2 / self.batch_size * (y_batch - self.predict(x_batch)).T @ x_batch
